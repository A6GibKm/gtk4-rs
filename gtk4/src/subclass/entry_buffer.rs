...
unsafe extern "C" fn entry_buffer_insert_text<T: EntryBufferImpl>(
    ...
    let mut char_indices = text.char_indices().map(|(i, _char)| i);
    let pos_end = if n_chars != u32::MAX {
        char_indices.nth(n_chars as usize)
    } else {
        char_indices.last()
    }
    .expect("The passed characters to EntryBuffer contains less characters than what's used");

    imp.insert_text(wrap.unsafe_cast_ref(), position, &text[0..(pos_end + 1)])
}
...
unsafe extern "C" fn entry_buffer_inserted_text<T: EntryBufferImpl>(
    ...
    let mut char_indices = text.char_indices().map(|(i, _char)| i);
    let pos_end = if length != u32::MAX {
        char_indices.nth(length as usize)
    } else {
        char_indices.last()
    }
    .expect("The passed characters to EntryBuffer contains less characters than what's used");

    imp.inserted_text(wrap.unsafe_cast_ref(), position, &text[0..(pos_end + 1)])
}
